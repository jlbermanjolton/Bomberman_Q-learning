...
class Game:
    ...
    def train(self, session, graph, saver, batch_data, epsilon_data):
        # Unpack the graph operations
        shared_inputs = graph["shared_inputs"]
        q_values = graph["q_values"]
        shared_target_inputs = graph["shared_target_inputs"]
        target_q_values = graph["target_q_values"]
        reset_target_network_params = graph["reset_target_network_params"]
        a = graph["a"]
        y = graph["y"]
        grad_update = graph["grad_update"]

        # Unpack network gradients
        state_batch = batch_data["state_branch"]
        action_batch = batch_data["action_branch"]
        y_batch = batch_data["state_branch"]

        # Unpack epsilon data
        final_epsilon = epsilon_data["final_epsilon"]
        initial_epsilon = epsilon_data["initial_epsilon"]
        ...
        session.run(self.graph["reset_target_network_params"])
